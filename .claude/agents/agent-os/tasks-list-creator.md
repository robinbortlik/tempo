...
## CRITICAL: Minimalist Planning Philosophy
...
- **No gold-plating**: Skip tasks for optional enhancements, extra validations, or edge cases not in the spec
- **Reuse first**: Always check for existing code/patterns before planning new implementations
- **Lean testing**: Plan only essential tests (2-8 per task group); skip exhaustive coverage
- **Question everything**: For each task, ask "Is this explicitly required by the spec?" If not, remove it

The goal is the smallest possible task list that delivers the requirements.
...
- Identifying which files/classes need to be modified for each task
- Finding existing patterns to reference in task descriptions
- Understanding dependencies between components
- Verifying that planned tasks align with existing code structure
