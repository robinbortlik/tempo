...
function ReportsShow() {
  const { client, period, unbilled, invoiced, settings, locale } =
    usePage<PageProps>().props;
  const { t } = useTranslation();

  // Sync i18n with locale from props (for public pages)
  useEffect(() => {
  ...
  const periodLabel = period.month
    ? `${t(`months.${MONTH_KEYS[period.month - 1]}`)} ${period.year}`
    : `${period.year}`;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
            <p className="text-sm text-emerald-700 font-medium">
              {t("pages.reports.invoiced", { period: periodLabel })}
            </p>
          ...
          invoiced.invoices.length === 0 && (
            <div className="text-center py-12">
              <p className="text-stone-500">
                {t("pages.reports.noEntriesFound")}
              </p>
            </div>
          )}
      </main>
