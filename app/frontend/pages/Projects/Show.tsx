...
export default function ProjectShow() {
  ...
  const handleToggleActive = () => {
    setIsToggling(true);
    router.patch(`/projects/${project.id}/toggle_active`, {}, {
        ...
        <div className="mb-6">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-stone-500">
                  {project.client_name} {" \u00B7 "}
                  {formatRate(project.effective_hourly_rate, project.client_currency)}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {isToggling ? "Updating..." : project.active ? "Deactivate" : "Activate"}
              </Button>
              <Button
                variant="outline"
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This action cannot be undone. This will permanently delete the project. Projects with invoiced time entries cannot be deleted.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
          ...
          <Table>
            <TableHeader>
              <TableRow className="text-left text-sm text-stone-500 border-b border-stone-200">
                ...
                <TableHead className="px-6 py-4 font-medium">Description</TableHead>
                <TableHead className="px-6 py-4 font-medium text-right">Hours</TableHead>
                <TableHead className="px-6 py-4 font-medium text-right">Amount</TableHead>
            ...
            <TableBody>
              {time_entries.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="px-6 py-8 text-center text-stone-500">
                    No time entries yet.
                  </TableCell>
                </TableRow>
              ) : (
                time_entries.map((entry) => (
                  <TableRow key={entry.id} className="border-b border-stone-100">
                    <TableCell className="px-6 py-4 text-stone-600">
                      {formatDate(entry.date)}
                    </TableCell>
                    <TableCell className="px-6 py-4 text-stone-900">
                      {entry.description || <span className="text-stone-400">No description</span>}
                    </TableCell>
                    <TableCell className="px-6 py-4 text-right tabular-nums text-stone-900">
                      {entry.hours}h
                    </TableCell>
                    <TableCell className="px-6 py-4 text-right tabular-nums text-stone-900">
                      {formatCurrency(entry.calculated_amount, project.client_currency)}
                    </TableCell>
